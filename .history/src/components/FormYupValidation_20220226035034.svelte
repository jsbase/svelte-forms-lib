...
const add = () => {
    ...
    $errors.users = Array.from($errors.users).concat({ name: "", email: "" }).toString();
};
...
<form>
    {#each $form.users as user, j}
        <div class="form-group">
            <div>
                <input name={`user[${j}].name`} placeholder="name"
                    on:change={handleChange} on:blur={handleChange} bind:value={$form.users[j].name}
                />
            ...
            <div>
                <input placeholder="email" name={`user[${j}].email`}
